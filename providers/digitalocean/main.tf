...
resource "null_resource" "databased_servers" {
  # Changes to any instance of the cluster requires re-provisioning
  triggers {
    cluster_instance_ids = "${join(",", digitalocean_droplet.web.*.id)}"
  }
  ...
  provisioner "local-exec" {
    ...
    command = "$INFRASTRUCTURE_ROOT_DIR/provisioners/mysql-box/provision_box.sh"
  ...
  depends_on = ["null_resource.lamped_servers"]
